...
SI.Game = function() {
	...
	this.pressedKey = null; //currently pressed key (starts at null)

	this.rocketsByPlayer = []; //a list of rockets fired by the player
	this.rocketsByEnemies = []; //a list of rockets fired by the enemies
	this.playerShip = null;; 
	this.points = 0; //score
	this.lives = 2; //remaining lives
	this.enemies = {}; 
	this.frames = 0;
	// collison detector
	this.detector = null;
...
SI.Game.prototype.start = function () {
	this.initializeCanvas(); //set up canvas
	this.attachKeyboardEvents(); //attach keyboard events
	this.initializeGame(); //initialize game components
	this.drawAllElements(); //begin drawing loop
}
...
SI.Game.prototype.initializeCanvas = function () {
	...
	this.ctx = $canvas[0].getContext('2d'); //set drawing context to 2d
	this.ctx.canvas.height = SI.Sizes.height; //set canvas height
	this.ctx.canvas.width = SI.Sizes.width; //set canvas width
	this.ctx.font = SI.Sizes.font; //set font
}

SI.Game.prototype.attachKeyboardEvents = function() {
	var self = this;
	$(document).keydown(function (e) {
		self.onKeyDown(e); //events that happen when down key pressed
	});
	$(document).keyup(function (e) {
		e.preventDefault();
		self.onKeyUp(e); //events that happen when up key pressed
	});
    //rest of this block handles touchscreen capabilities
...
SI.Game.prototype.initializeGame = function () {
	...
	//start initializing all game elements
	...
	//end initialiaing all game elements
}

//checks whether the game should end based on the state of the board
...
/*
 * Creates even grid of enemy ships
 * Parameters
 * rows - number of rows of ships
 * columns - number of columns of ships
 */
 ...
 */
SI.Game.prototype.lockRocketLauncher = function () { //locks firing ability
	this.okToFire = false;
}
SI.Game.prototype.freeRocketLauncher = function () { //unlocks firing ability
	this.okToFire = true;
}

/*
 * Deletes Rockets that have exploded, creates explosions at their locations
 */
SI.Game.prototype.deleteExplodedRockets = function () {
	var aliveRockets = [];
	//functionality for rockeys by player
	...
	//functionality for rockets by enemies
	aliveRockets = [];
...
SI.Game.prototype.deleteExplodedEnemyShips = function () {
	...
	for (var i = 0; i < toDelete.length; i += 1) {
		...
		this.points += SI.Sizes.pointModifer; //increase points
	}
...
//remove the done explosions from the list
...
//checks if the ship has been hit by a rocket
...
//if allowed, allow player to fire rocket
...
/*
 * Change the current key pressed
 */
SI.Game.prototype.onKeyDown = function (e) {
	if(e.which == SI.Keys.Right)
		this.moveRight = true; //if right key pressed, move ship right
	else if(e.which == SI.Keys.Left)
		this.moveLeft = true; //if left key pressed, move ship left
	else if(e.which == SI.Keys.Up) {
		this.launchPlayerRocket(); //if up key pressed, launch rocket
	}
}
...
SI.Game.prototype.movePlayerShip = function () {
	if(this.moveRight) { //move ship right
		this.playerShip.move(SI.Sizes.playerStep, 0);
	}
	else if(this.moveLeft) { //move ship left
		this.playerShip.move(-SI.Sizes.playerStep, 0);
	}
}
...
SI.Game.prototype.moveEnemyShips = function () {
	...
	for (var i = lastRow; i >= 0; i -= 1) {
		if(this.enemies.ships[i][0].x <= SI.Sizes.leftMargin) { 
			this.enemies.directions[i] = SI.Directions.Right; //switch to moving to the right
			if(i == lastRow) {
				dropDown = true;
			}
		}
		else if(this.enemies.ships[i][this.enemies.ships[i].length - 1].x + this.enemies.ships[i][0].width >= SI.Sizes.rightMargin) {
			this.enemies.directions[i] = SI.Directions.Left; //switch to moving to the left
			if(i == lastRow) {
				dropDown = true; //condense enemies
			}
		}
